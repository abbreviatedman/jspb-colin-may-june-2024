...
// 9. Get the section that serves as the wrapper of the interface:
...
// divvy.textContent = "Dynamic Divvy";

// 12. Use appendChild() to output the div inside the section:
// section.appendChild(divvy);

// 13. Reload the page. In the upper left corner of the section there should be one smallish box that says "Dynamic Divvy".
...
for (let i = 0; i < animals.length; i++) {
  ...
  let animal = animals[i]; // simplify current animal

  let pic = new Image(); // Image obj for animal pic
  pic.src = `images/animals/${animal.eng}.jpg`;
  pic.className = "animal-pic";
  divvy.appendChild(pic);

  const inputBox = document.createElement("input");
  inputBox.type = "search"; // as opposed to "text"
  inputBox.placeholder = "animal";
  // assign a bunch of the obj properties to inputBox
  // these can be accessed by this.eng, this.chi, etc
  // inside the function that is called when user hits
  // enter from inside the box

  inputBox.eng = animal.eng;
  inputBox.chi = animal.chi;
  inputBox.also = animal.also;
  inputBox.i = i;
  // call checkSpelling function when user hits enter
  // or tab from inside the input box
...
function checkSpelling() {
  // check user input against all 3 accepted spellings
  if (
    this.value == this.eng ||
    this.value == this.also ||
    this.value == this.chi
  ) {
    this.style.backgroundColor = "#0b0"; // green
  } else {
    this.style.backgroundColor = "#a21"; // red
  }
